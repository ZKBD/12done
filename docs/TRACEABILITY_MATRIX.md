...
| 2026-01-02 | Claude | UI Bug Fix - Multiple Listing Type Badges: Fixed property-card.tsx to display ALL listing type badges instead of only the first one; Properties with multiple listing types (e.g., FOR_SALE + LONG_TERM_RENT) now show all badges side-by-side; Verified fix deployed to production at https://12done.com |
| 2026-01-02 | Claude | UI Bug Fix - Notifications Page Crash: Fixed notifications/page.tsx to filter out invalid notification objects (null or missing type property) that caused runtime crash when mapping over notification array; Added null-safe filter: `.filter((n): n is Notification => n != null && n.type != null)` |
| 2026-01-02 | Claude | UI Bug Fix - Search Agents Modal Crash: Fixed search-agents/page.tsx SelectItem components that had empty string values causing "A <SelectItem /> must have a value prop" error; Changed `value=""` to `value="none"` for placeholder options in property type, bedrooms, and bathrooms selects |
| 2026-01-02 | Claude | Feature - User Settings Page: Created comprehensive settings page at /dashboard/settings with 4 tabs (Profile, Notifications, Security, Account); Profile tab includes name, phone, address fields with react-hook-form + zod validation; Notifications tab has email/push/marketing toggles; Security tab shows password change and 2FA status; Account tab has data export and account deletion with confirmation dialog |
